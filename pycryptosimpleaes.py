...
class Win1:
  ...
  def new_window(self,  _class):
    ...
    _class(self.new, self.encryptionManager)


class Win2:
  def encrypts(self):
    message = self.t1.get("1.0", tk.END) # Mensagem a ser criptografada
    secret_key= self.tkey.get() # Chave secreta de 16 bytes

    ciphertext = self.encryptionManager.encrypts(message, secret_key)

    self.t2.delete('1.0', tk.END)
    self.t2.insert("1.0", ciphertext)

  def __init__(self, master, encryptionManager):
    self.encryptionManager = encryptionManager
    ...
    self.lkey = tk.Label(self.master, text='Secret key - 16 bytes', font=("times", 16, "bold"), bg="palegreen", fg='red2')
    self.lkey.pack(expand="yes")
    self.tkey = tk.Entry(self.master, width=16, borderwidth=1, relief="solid")
...
class Win3:
  ...
  def decrypts(self):
    ...
    secret_key = self.tkey.get()

    originalMessage = self.encryptionManager.decrypts(cipherText, secret_key)

    self.t2.delete('1.0', tk.END)
    self.t2.insert("1.0", originalMessage)
  ...
  def __init__(self, master, encryptionManager):
    ...
    self.lkey = tk.Label(self.master, text='Secret key - 16 bytes', font=("times", 16, "bold"), bg="pink", fg='blue')
    self.lkey.pack(expand="yes")
    self.tkey = tk.Entry(self.master, width=16, borderwidth=1, relief="solid")
    self.tkey.pack(expand="yes")
